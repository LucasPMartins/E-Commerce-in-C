...
int lista_clientes_vazia(lista_clientes *l){
    if(l == NULL) return 1;
    if(l->inicio == NULL && l->total_clientes == 0) return 0;
    else return 2;
}

int lista_carrinho_vazia(cliente *l){
    if(l == NULL) return 1;
    if(l->carrinho_inicio == 0 && l->total_carrinho == 0) return 0;
    else return 2;
}

int lista_comprados_vazia(cliente *l){
    if(l == NULL) return 1;
    if(l->comprados_inicio == NULL && l->total_comprados == 0) return 0;
    else return 2;
}

int verifica_cliente(lista_clientes *l,cadastro it){
    if(l == NULL) return 1;
    if(lista_clientes_vazia(l) == 0 ) return insere_novo_cliente(l,it);
...
int insere_novo_cliente(lista_clientes *l,cadastro it){
    ...
    user->prox = l->inicio;
    user->ant = NULL;
    if(l->inicio != NULL) l->inicio->ant = user;
    l->inicio = user;
    l->total_clientes++;
    return 0;
}
...
int insere_nova_compra(lista_clientes *l,cadastro it,produtos p){
    ...
    while(search != NULL){
        if(strcmp(it.nome,search->valor.cadastro.nome) == 0 && strcmp(it.senha,search->valor.cadastro.senha) == 0){
            ...
            no->ant = NULL;
            if(search->valor.comprados_inicio != NULL) search->valor.comprados_inicio->ant = no;
...
int insere_novo_carrinho(lista_clientes *l,cadastro it,produtos p){
    ...
    while(search != NULL){
        if(strcmp(it.nome,search->valor.cadastro.nome) == 0 && strcmp(it.senha,search->valor.cadastro.senha) == 0){
            ...
            no->ant = NULL;
            if(search->valor.carrinho_inicio != NULL) search->valor.carrinho_inicio->ant = no;
...
int remove_do_carrinho(lista_clientes *l,cadastro it,int pos){
    ...
    while(search != NULL){
        if(strcmp(it.nome,search->valor.cadastro.nome) == 0 && strcmp(it.senha,search->valor.cadastro.senha) == 0){
            if(lista_carrinho_vazia(&search->valor) == 0) return 3; // carrinho vazio
            if(pos > search->valor.total_carrinho || pos < 0) return 4;
...
void mostra_lista_clientes(lista_clientes *l){
    if(l == NULL){ printf("Lista de clientes nao criada!\n"); return;}
    if(lista_clientes_vazia(l) == 0) { printf("Lista de clientes vazia!\n"); return;}
    no_clientes *search = l->inicio;
    int i = 0;
    while(search != NULL){
        ...
        if(search->valor.comprados_inicio == NULL ) printf("Nenhum produto foi comprado!\n");
        free(prod);
        j = 0;
        ...
        if(search->valor.carrinho_inicio == NULL) printf("Carrinho vazio!\n");
        printf("\n");
        i++;
        search = search->prox; 
    }
